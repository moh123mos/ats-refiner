<template>
  <div class="resume-template bg-white max-w-[210mm] mx-auto shadow-lg" id="resume-content">
    ...
    <div class="bg-slate-900 px-8 py-7 text-white">
      ...
      <div class="w-16 h-1 bg-amber-500 mt-2 mb-3"></div>
    ...
    <div class="p-8">
      <!-- Summary -->
      <section v-if="resume.summary" class="mb-6 bg-slate-50 border-l-4 border-amber-500 p-4">
      ...
      <section v-if="resume.experience.length" class="mb-6">
        ...
        <div v-for="(exp, i) in resume.experience" :key="i" class="mb-5 last:mb-0">
          <div class="flex justify-between items-start">
            <div>
              <h3 class="text-sm font-bold text-slate-900 uppercase">{{ exp.title }}</h3>
              <p class="text-sm font-medium text-amber-700">{{ exp.company }}<span v-if="exp.location" class="text-slate-500 font-normal"> | {{ exp.location }}</span></p>
            </div>
          ...
          <ul class="mt-2 space-y-1.5">
            <li v-for="(bullet, j) in exp.bullets" :key="j" class="text-sm text-slate-600 flex items-start gap-2">
              <span class="w-1 h-1 bg-amber-500 mt-2 flex-shrink-0"></span>
              <span>{{ bullet }}</span>
            </li>
          </ul>
        </div>
        ...
        <div class="col-span-2 space-y-6">
          ...
          <section v-if="resume.projects.length">
            ...
            <div v-for="(proj, i) in resume.projects" :key="i" class="mb-3 last:mb-0">
              <h3 class="text-sm font-bold text-slate-800">{{ proj.name }}</h3>
              <p v-if="proj.technologies.length" class="text-xs text-amber-600 mb-1">{{ proj.technologies.join(' | ') }}</p>
        ...
        <div class="space-y-6">
          ...
          <section v-if="resume.education.length">
            ...
            <div v-for="(edu, i) in resume.education" :key="i" class="mb-3 last:mb-0">
              ...
              <p class="text-xs text-slate-400">{{ edu.graduationDate }}</p>
            </div>
          </section>
          ...
          <section v-if="resume.skills.length">
            ...
            <div class="space-y-1">
              <div v-for="skill in resume.skills" :key="skill" class="text-xs text-slate-600 flex items-center gap-2">
                <span class="w-1 h-1 bg-amber-500 flex-shrink-0"></span>
                {{ skill }}
              </div>
            </div>
          </section>
          ...
          <section v-if="resume.certifications.length">
            ...
            <div v-for="(cert, i) in resume.certifications" :key="i" class="mb-2 last:mb-0">
              <p class="text-xs font-medium text-slate-700">{{ cert.name }}</p>
          ...
          <section v-if="resume.languages.length">
            ...
            <div class="space-y-1">
              <p v-for="lang in resume.languages" :key="lang" class="text-xs text-slate-600">{{ lang }}</p>
            </div>
          </section>
        </div>
      </div>
