  ...
  <div class="resume-template bg-white max-w-[210mm] mx-auto shadow-lg" id="resume-content">
    ...
    <div class="bg-gradient-to-r from-emerald-600 to-teal-600 px-8 py-6 text-white">
      ...
      <div class="flex items-center flex-wrap gap-x-5 gap-y-1 mt-2 text-sm text-emerald-100">
        <span v-if="resume.contact.email" class="flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
          {{ resume.contact.email }}
        </span>
        <span v-if="resume.contact.phone" class="flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
          {{ resume.contact.phone }}
        </span>
        <span v-if="resume.contact.location" class="flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
          {{ resume.contact.location }}
        </span>
        <span v-if="resume.contact.linkedin" class="flex items-center gap-1.5">
    ...
    <div class="p-8">
      ...
      <section v-if="resume.experience.length" class="mb-6">
        ...
        <div v-for="(exp, i) in resume.experience" :key="i" class="mb-4 last:mb-0 pl-4 border-l-2 border-emerald-200">
          <div class="flex justify-between items-start">
            <div>
              <h3 class="text-sm font-bold text-slate-800">{{ exp.title }}</h3>
              <p class="text-sm text-emerald-600 font-medium">{{ exp.company }}<span v-if="exp.location" class="text-slate-400 font-normal"> • {{ exp.location }}</span></p>
            </div>
            <span class="text-xs text-slate-400 bg-slate-50 px-2 py-0.5 rounded whitespace-nowrap">{{ exp.startDate }} – {{ exp.endDate }}</span>
          </div>
          <ul class="mt-2 space-y-1">
            <li v-for="(bullet, j) in exp.bullets" :key="j" class="text-sm text-slate-600 flex items-start gap-2">
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        <section v-if="resume.education.length">
          ...
          <div v-for="(edu, i) in resume.education" :key="i" class="mb-3 last:mb-0">
            ...
            <p class="text-xs text-slate-400">{{ edu.graduationDate }}<span v-if="edu.gpa"> • GPA: {{ edu.gpa }}</span></p>
          </div>
        </section>
        ...
        <section v-if="resume.skills.length">
          ...
          <div class="flex flex-wrap gap-1.5">
            <span v-for="skill in resume.skills" :key="skill" class="text-xs px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-100">
              {{ skill }}
            </span>
          </div>
        </section>
      ...
      <!-- Projects -->
      <section v-if="resume.projects.length" class="mt-6">
        <h2 class="text-xs font-bold text-emerald-700 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
          <span class="w-5 h-0.5 bg-emerald-500 rounded-full"></span>
          Projects
        </h2>
        <div v-for="(proj, i) in resume.projects" :key="i" class="mb-3 last:mb-0">
          <h3 class="text-sm font-bold text-slate-800">
            {{ proj.name }}
            <span v-if="proj.technologies.length" class="font-normal text-emerald-500 text-xs ml-1">{{ proj.technologies.join(', ') }}</span>
          </h3>
          ...
          <ul v-if="proj.bullets.length" class="mt-1 space-y-0.5">
            <li v-for="(b, j) in proj.bullets" :key="j" class="text-sm text-slate-600 flex items-start gap-2">
              <span class="w-1.5 h-1.5 bg-emerald-400 rounded-full mt-1.5 flex-shrink-0"></span>
              <span>{{ b }}</span>
            </li>
          </ul>
      ...
      <div class="grid grid-cols-2 gap-6 mt-6" v-if="resume.certifications.length || resume.languages.length">
        <section v-if="resume.certifications.length">
          <h2 class="text-xs font-bold text-emerald-700 uppercase tracking-[0.2em] mb-2 flex items-center gap-2">
            <span class="w-5 h-0.5 bg-emerald-500 rounded-full"></span>
            Certifications
          </h2>
          <div v-for="(cert, i) in resume.certifications" :key="i" class="text-sm mb-1">
